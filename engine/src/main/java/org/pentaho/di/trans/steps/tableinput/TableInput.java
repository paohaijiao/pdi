    ...
    if ( super.init( smi, sdi ) ) {
      ...
      try {
        ...
        if ( getTransMeta().isUsingUniqueConnections() ) {
          System.out.println("########################################isUsingUniqueConnections#####################");
          synchronized ( getTrans() ) {
            System.out.println("########################################data.db.connect#####################");
            data.db.connect( getTrans().getTransactionId(), getPartitionID() );
          }
        } else {
          System.out.println("######################################## data.db.connect( getPartitionID() );#####################");
          data.db.connect( getPartitionID() );
        }

        if ( meta.getDatabaseMeta().isRequiringTransactionsOnQueries() ) {
          System.out.println("########################################   data.db.setCommit( 100 );#####################");
          data.db.setCommit( 100 ); // needed for PGSQL it seems...
        }
        if ( log.isDetailed() ) {
          System.out.println("########################################   log.isDetailed()#####################");
          logDetailed( "Connected to database..." );
        }

        return true;
      } catch ( KettleException e ) {
        logError( "错误产生了注意异常: " + e.getMessage() );
        setErrors( 1 );
        stopAll();
      }
    }
